######################################################################################################
# Script: Limma Analysis of 72h Proteome and IRF4-Pulldown (Interactome) Data
...
# maxpep >= 2 
# runfilter: 
# th17_more_than_7 <- c(limma_wholeProt_input_raw[anno_wholeProt$Cell == "Th17"] %>% apply(1, count_na)) > 1
# treg_more_than_7 <- c(limma_wholeProt_input_raw[anno_wholeProt$Cell == "Treg"] %>% apply(1, count_na)) > 1
# results_prot <- results_prot[rownames(limma_wholeProt_input_raw[!(th17_more_than_7 & treg_more_than_7),]), ] # remove Proteins, that do not have at least one condition with more than 8 non-NA samples 
# this means that we generate a boolean vector for both Th17 and Tregs
# holding a TRUE everytime a Protein has more than 1 NA in either of the groups.
# The two vectors are combined by a logical & and negated resulting in a dataset
# with only Proteins holding more than 8 non-NA values in the raw intensities.
# adj.P <= 0.05
# logFC > 0.5

outdir <- "output_new/mouse_72h" #_max_pep-3_run_filter_6"
dir.create(outdir)
...
proteome_Th17vTreg_base <- read.xlsx(file.path(data.folder, "2022-143_Proteom_Th17_TReg_Bio_3Rep_relaxed-prot-inf_MaxLFQ_Intensities_UD.xlsx"), sheet = 1)
proteome_Th17vTreg_base <- proteome_Th17vTreg_base %>% dplyr::select(-X1)
proteome_Th17vTreg_base$prot.names <- str_remove(proteome_Th17vTreg_base$prot.names, "_MOUSE")
...
experiments_wholeProt <- colnames(proteome_Th17vTreg)
...
celltypes_prot <- c(rep("Treg", times=length(experiments_wholeProt)/2) ,rep("Th17", times=length(experiments_wholeProt)/2))
anno_wholeProt["Cell"] <- factor(celltypes_prot, levels = c("Th17", "Treg"))
anno_wholeProt_row <- data.frame(row.names =proteome_Th17vTreg$prot.names,"max_pep"=proteome_Th17vTreg$max_pep)
wholeProt_pd_scores <-  proteome_Th17vTreg %>%  dplyr::select("prot.names", all_of(experiments_wholeProt)) %>% column_to_rownames("prot.names")
...
dep_input_raw <- make_se(unique_prot[unique_prot$max_pep >= max_pep_thresh_prot,], bool_col, experimental_design_prot)
...
ggsave(file = 'output_new/mouse_72h/volcanoplot_wholeprot.pdf', width=2000, height=2000, units = "px", dpi=300)
...
th17_more_than_7 <- c(limma_wholeProt_input_raw[anno_wholeProt$Cell == "Th17"] %>% apply(1, count_na)) > max_NAs
treg_more_than_7 <- c(limma_wholeProt_input_raw[anno_wholeProt$Cell == "Treg"] %>% apply(1, count_na)) > max_NAs
...
write.xlsx(results_prot, "output_new/mouse_72h/results_limma_proteome.xlsx", rowNames=T) # prot names are missing :S

#### output
out_excel <- read.xlsx(file.path(data.folder, "2022-143_Proteom_Th17_TReg_Bio_3Rep_relaxed-prot-inf_MaxLFQ_Intensities_UD.xlsx"), sheet = 4) #, colNames = F)
out_excel$prot.names <- str_remove(out_excel$prot.names, "_MOUSE")

prot7_out <- as_tibble(results_prot_comp, rownames = "prot.names") %>%
  dplyr::select("prot.names", "logFC", "adj.P.Val") %>%
  mutate(significant_limma = ifelse(
    logFC > 0 & prot.names %in% rownames(results_prot), "th17_enriched", 
      ifelse(logFC < 0 & prot.names %in% rownames(results_prot), "treg_enriched", "ns")))
out_excel <- out_excel %>% left_join(prot7_out, by="prot.names")

write.xlsx(out_excel, "output_new/mouse_72h/results_proteome.xlsx", rowNames=F, colNames=T) 
...
# maxpep >= 3 
# runfilter: 
# BIO_more_than_7_treg <- c(limma_Treg_input_raw[anno_Treg$Type == "Bio"] %>% apply(1, count_na)) > 1
# results_paired_treg <- results_paired_treg[rownames(limma_Treg_input_raw[!(BIO_more_than_7_treg),]), ] # remove Proteins, that do not have at least one condition with more than 8 non-NA samples 
...
pulldown_combi <- read.xlsx(file.path(data.folder, "2021-143_MaxLFQ_Intensities_Th17_TReg_Pulldown_together_MBR_nocrossnorm_Core4_UD.xlsx"), sheet = 3, colNames = F)
...
pulldown_combi_base$prot.names <-  str_remove(pulldown_combi_base$prot.names, "_MOUSE")
...
celltypes_prot <- c(rep("Treg", times=length(experiments_combi)/2) ,rep("Th17", times=length(experiments_combi)/2))
...
anno_Treg <- data.frame(row.names =experiments_Treg,"Type"=str_extract(experiments_Treg, "WT|Bio"), "Exp"=str_extract(experiments_Treg, "Exp..[0-9]*"))
anno_Th17 <- data.frame(row.names =experiments_Th17,"Type"=str_extract(experiments_Th17, "WT|Bio"), "Exp"=str_extract(experiments_Th17, "Exp..[0-9]*"))
...
EnhancedVolcano(results_paired_treg_comp,
                ...
                subtitle = "imputed data (min + knn)") 
ggsave(file = 'output_new/mouse_72h/paired_volcanoplot_treg.pdf', width=2000, height=2000, units = "px", dpi=300)


BIO_more_than_7_treg <- c(limma_Treg_input_raw[anno_Treg$Type == "Bio"] %>% apply(1, count_na)) > max_NAs
...
write.xlsx(results_paired_treg, "output_new/mouse_72h/paired_results_limma_iact_Treg.xlsx", rowNames=T) # prot names are missing :S
...
EnhancedVolcano(results_paired_th17_comp,
                ...
                subtitle = "imputed data (min + knn)") 
ggsave(file = 'output_new/mouse_72h/paired_volcanoplot_th17.pdf', width=2000, height=2000, units = "px", dpi=300)

BIO_more_than_7_th17 <- c(limma_th17_input_raw[anno_Th17$Type == "Bio"] %>% apply(1, count_na)) > max_NAs
# WT_more_than_7 <- c(limma_th17_input_raw[anno_Th17$Type == "WT"] %>% apply(1, count_na)) > 1

results_paired_th17 <- results_paired_th17[rownames(limma_th17_input_raw[!(BIO_more_than_7_th17),]), ] # remove Proteins, that do not have at least one condition with more than 7 nonNA samples 
...
write.xlsx(results_paired_th17, "output_new/mouse_72h/paired_results_limma_iact_th17.xlsx", row.names=T) # prot names are missing :S
...
ggsave(file = 'output_new/mouse_72h/paired_volcanoplot_combi.pdf', width=2000, height=2000, units = "px", dpi=300)

# results_paired_combi <- results_paired_combi[rownames(limma_combi_input_raw[!(BIO_more_than_7 & WT_more_than_7),]), ] # remove Proteins, that do not have at least one condition with more than 8 non-NA samples 
...
results_paired_combi <- results_paired_combi[results_paired_combi$logFC >= 1,] # pulldown higher than 0.5! mind. 1? 
write.xlsx(results_paired_combi, "output_new/mouse_72h/paired_results_limma_iact_combi.xlsx", rowNames=T) # prot names are missing :S
...
out_excel <- out_excel %>% left_join(th17_out, by="prot.names") %>% 
  ...
  left_join(as_tibble(BIO_more_than_7_th17, rownames="prot.names") %>% dplyr::rename(th17_Bio_more_than_1_NA=value)) %>%
  left_join(as_tibble(BIO_more_than_7_treg, rownames="prot.names") %>% dplyr::rename(treg_Bio_more_than_1_NA=value))
...
write.xlsx(out_excel, "output_new/mouse_72h/paired_mega_table_iact.xlsx", rowNames=F, colNames=F)
...
write.xlsx(out_excel, "output_new/mouse_72h/paired_mega_table_iact_with_combi.xlsx", rowNames=F, colNames=F)


# write imputed data to file
write.xlsx(as.data.frame(limma_th17_input), "output_new/mouse_72h/th17_data_imputed.xlsx", rowNames=T)
write.xlsx(as.data.frame(limma_treg_input), "output_new/mouse_72h/treg_data_imputed.xlsx", rowNames=T)
...
design_subset <- model.matrix(~0+Cell, data = anno_subset, level=Bio)
contr.matrix <- makeContrasts(
  Th17vsTreg = CellTh17 - CellTreg,
...
ggsave(file = 'output_new/mouse_72h/paired_volcanoplot_bioonly.pdf', width=3000, height=3000, units = "px", dpi=300)
...
write.xlsx(results_paired_subset %>%
           ...
           "output_new/mouse_72h/paired_results_limma_iact_bioonly.xlsx")
...
# print(hm)
ggsave(file = 'output_new/mouse_72h/heatmap_interactome_Th17.pdf', plot=hm, width=1650, height=3000, units="px", dpi=300)

rows_txt <- rownames(hm_inp[hm$tree_row$order,])
write.csv(rows_txt, "output_new/mouse_72h/heatmap_interactome_Th17.csv")
...
hm <- pheatmap(hm_inp, cluster_rows=T, show_rownames=F,show_colnames=F,annotation_names_row=F, color=mat_colors,
               ...
               annotation_row = anno_combi_row , annotation_legend = T)
# print(hm)
ggsave(file = 'output_new/mouse_72h/heatmap_interactome_Th17_zscore.pdf', plot=hm, width=1650, height=3000, units="px", dpi=300)

rows_txt <- rownames(hm_inp[hm$tree_row$order,])
write.csv(rows_txt, "output_new/mouse_72h/heatmap_interactome_Th17_zscore.csv")
...
hm <- pheatmap(hm_inp, cluster_rows=T, show_rownames=F,show_colnames=F,annotation_names_row=F,color=mat_colors,
               ...
               annotation_row = anno_combi_row , annotation_legend = T)
# print(hm)
ggsave(file = 'output_new/mouse_72h/heatmap_interactome_Treg.pdf', plot=hm, width=1650, height=3000, units="px", dpi=300)

rows_txt <- rownames(hm_inp[hm$tree_row$order,])
write.csv(rows_txt, "output_new/mouse_72h/heatmap_interactome_Treg.csv")
...
hm <- pheatmap(hm_inp, cluster_rows=T, show_rownames=F,show_colnames=F,annotation_names_row=F,color=mat_colors,
               ...
               annotation_row = anno_combi_row , annotation_legend = T)
# print(hm)
ggsave(file = 'output_new/mouse_72h/heatmap_interactome_Treg_zscore.pdf', plot=hm, width=1650, height=3000, units="px", dpi=300)

rows_txt <- rownames(hm_inp[hm$tree_row$order,])
write.csv(rows_txt, "output_new/mouse_72h/heatmap_interactome_Treg_zscore.csv")
...
hm <- pheatmap(hm_inp, cluster_rows=T, show_rownames=F,show_colnames=F,annotation_names_row=F,color=mat_colors,
               ...
               annotation_row = anno_combi_row , annotation_legend = T)
# print(hm)
ggsave(file = 'output_new/mouse_72h/heatmap_interactome_Th17vsTreg_subset.pdf', plot=hm, width=1650, height=3000, units="px", dpi=300)

rows_txt <- rownames(hm_inp[hm$tree_row$order,])
write.csv(rows_txt, "output_new/mouse_72h/heatmap_interactome_Th17vsTreg_subset.csv")
...
ggsave(file = 'output_new/mouse_72h/heatmap_Proteome.pdf', plot=hm, width=1650, height=3000, units="px", dpi=300)
# print(hm)
rows_txt <- rownames(hm_inp[hm$tree_row$order,])
write.csv(rows_txt, "output_new/mouse_72h/heatmap_Proteome.csv")

# just the smallest of each
# plot gene tree
# pdf(file = 'output_new/mouse_72h/heatmap_Proteome_tree.pdf', width=1650, height=3000, res=300)
# plot(hm$tree_row)
# abline(h=40)
# dev.off()
...
hm <- pheatmap(hm_small, cluster_rows=T, show_rownames=F,show_colnames=F,annotation_names_row=F,color=mat_colors,
               ...
               annotation_row = anno_wholeProt_row , annotation_legend = T)
# print(hm)
ggsave(file = 'output_new/mouse_72h/heatmap_Proteome_sub.pdf', plot=hm, width=1650, height=3000, units="px", dpi=300)

rows_txt <- rownames(hm_small[hm$tree_row$order,])
write.csv(rows_txt, "output_new/mouse_72h/heatmap_Proteome_sub.csv")
...
hm <- pheatmap(hm_inp, cluster_rows=T, show_rownames=F,show_colnames=F,annotation_names_row=F,color=mat_colors,
               ...
               annotation_colors = colors_matrix,
               annotation_row = anno_wholeProt_row , annotation_legend = T)
ggsave(file = 'output_new/mouse_72h/heatmap_Proteome_zscore.pdf', plot=hm, width=1650, height=3000, units="px", dpi=300)

rows_txt <- rownames(hm_inp[hm$tree_row$order,])
write.csv(rows_txt, "output_new/mouse_72h/heatmap_Proteome_zscore.csv")
...
hm <- pheatmap(hm_inp, cluster_rows=T, show_rownames=F,show_colnames=F,annotation_names_row=F,color=mat_colors,
               ...
               annotation_colors = colors_matrix,
               annotation_row = anno_combi_row , annotation_legend = T)
ggsave(file = 'output_new/mouse_72h/heatmap_interactome_complete.pdf', plot=hm, width=1650, height=3000, units="px", dpi=300)

rows_txt <- rownames(hm_inp[hm$tree_row$order,])
write.csv(rows_txt, "output_new/mouse_72h/heatmap_interactome_complete.csv")
...
hm <- pheatmap(hm_inp, cluster_rows=T, show_rownames=F,show_colnames=F,annotation_names_row=F,color=mat_colors,
               ...
               annotation_colors = colors_matrix,
               annotation_row = anno_combi_row , annotation_legend = T)
ggsave(file = 'output_new/mouse_72h/heatmap_interactome.pdf', plot=hm, width=1650, height=3000, units="px", dpi=300)

rows_txt <- rownames(hm_inp[hm$tree_row$order,])
write.csv(rows_txt, "output_new/mouse_72h/heatmap_interactome.csv")
...
# save
ggsave("output_new/mouse_72h/pca_all.pdf", plot=pca.plot, width = 5, height = 5, dpi=300)
...
# save
ggsave("output_new/mouse_72h/pca_interactome.pdf", plot=pca.plot, width = 5, height = 5, dpi=300)
...
pca.plot <- pca_plot_df_all %>%  
  ...
  scale_color_manual(values=c("#9C0E0F","#3E8EB9"))
# save
ggsave("output_new/mouse_72h/pca_interactome_noExp26.pdf", plot=pca.plot, width = 5, height = 5, dpi=300)
...
pca.plot <- pca_plot_df_all %>%  
  ...
  scale_color_manual(values=c("#9C0E0F","#3E8EB9"))
# save
ggsave("output_new/mouse_72h/pca_interactome_bio_only.pdf", plot=pca.plot, width = 5, height = 5, dpi=300)

################################################################################
# pca Proteome high and low quantiles of variance only
...
res.pca <- PCA(pca.table, scale.unit = TRUE, ncp = 5, graph = FALSE)
variance <- round(as.data.frame(res.pca$eig)["percentage of variance"], 2)
...
pca.plot <- pca_plot_df_proteome %>% 
  ...
  geom_point(aes(color = `Cell`, shape=`Type`)) +
  scale_color_manual(values=c("#9C0E0F","#3E8EB9"))
ggsave("output_new/mouse_72h/pca_proteome_imp_only_high+low_quantile.pdf", plot=pca.plot, width = 5, height = 5, dpi=300)

# PCA complete proteome
pca.table <- limma_wholeProt_input
...
pca.plot <- pca_plot_df_proteome %>% 
  ggplot(aes(x= Dim.1, y= Dim.2)) +
  geom_point(aes(color = `Cell`, shape=`Type`)) +
  labs(x = paste0("Dim 1 (", variance[1,], "%)"),
       y = paste0("Dim 2 (", variance[2,], "%)")) +
  # geom_text_repel(aes(label=id)) +
  scale_color_manual(values=c("#9C0E0F","#3E8EB9"))
ggsave("output_new/mouse_72h/pca_proteome_imp.pdf", plot=pca.plot, width = 5, height = 5, dpi=300)
...
pca.plot <- pca_plot_df_proteome %>% 
  ...
  scale_color_manual(values=c("#9C0E0F","#3E8EB9"))
ggsave("output_new/mouse_72h/pca_proteome_imp_annotated.pdf", plot=pca.plot, width = 5, height = 5, dpi=300)

################################################################################
# pca Interactome
pca.table <- 2^cbind(limma_th17_input, limma_treg_input) 
#  First find the desired quantile breaks for the entire matrix
qt <- quantile( pca.table , probs = c(p_val_thresh,0.99) , na.rm=T)
#  Next get a logical vector of the rows that have any values outside these breaks
rows <- apply( pca.table , 1 , function(x) any( x < qt[1] | x > qt[2] ) )
rows[is.na(rows)] <- FALSE # rmeove nas
#  Subset on this vector
...
pca.plot <- pca_plot_df_interactome %>% 
  ...
  scale_color_manual(values=c("#9C0E0F","#3E8EB9")) + 
  geom_text_repel(aes(label=id))
ggsave("output_new/mouse_72h/pca_interactome_only_high+low_quantile.pdf", plot=pca.plot, width = 5, height = 5, dpi=300)
...
pca.plot <- pca_plot_df_interactome %>% 
  ...
  geom_point(aes(color = `Cell`, shape=`Type`)) +
  scale_color_manual(values=c("#9C0E0F","#3E8EB9"))
ggsave("output_new/mouse_72h/pca_interactome_only_high+low_quantile_noExp26.pdf", plot=pca.plot, width = 5, height = 5, dpi=300)
...
venn_input_raw <- list(
  Interactome_Th17=rownames(Th17_pd_scores),
  Interactome_Treg=rownames(Treg_pd_scores),
  Proteome_Th17vsTreg=rownames(wholeProt_pd_scores)
)

ggVennDiagram(venn_input_raw, edge_size=0) +
  scale_fill_gradient(low="steelblue",high = "darkorange") +
  scale_x_continuous(expand = expansion(mult = .2))
ggsave("output_new/mouse_72h/venn_interactome-vs-Proteome-ttest.pdf")
...
ggsave('output_new/mouse_72h/log2ratios-data-withProt.pdf', plot = plt, width=12, height=10, dpi=300)
write.xlsx(plot_df, "output_new/mouse_72h/log2ratios-data-withProt.xlsx")
...
plt <- plot_df %>% 
  ...
  ggplot(aes(x=log2ratio_Proteome, y=log2ratio_Interactome)) +
  # geom_point( aes(color=`exceeds threshold`)) +
  geom_point( aes(color=Interactome, shape=Proteome), size=4) +
  geom_smooth(method = "lm", color="black") +
  geom_text_repel(aes(label=outside) )+ #, arrow = arrow(length = unit(0.02, "npc")) )+ #, hjust=1, vjust=1)+ 
  geom_segment(data = plot_df_quant, aes(x = log2ratio_Proteome, y = log2ratio_Interactome, xend = xend, yend = yend),
  ...
  coord_fixed(xlim = c(-10,10), ylim = c(-10,9))

# 
ggsave('output_new/mouse_72h/log2ratios-data-limma-iactanno.pdf', plot = plt, width=10, height=10, dpi=300)
write.xlsx(plot_df, "output_new/mouse_72h/log2ratios-data-limma.xlsx")