...
library(stringr)
require(ChIPseeker)
dir.create("output_new/mouse_chip_24h/")

# perm tests are outcommented during writing
perm_test_p <- 100

# load annotatio gtf and build txdb
...
# PUER, Natural_killer_cell, Macrophage, CD172+DC, NKT, B_cell_BM, RAW_macrophage
# "CD4+Treg",  "CD8+", CD4+, B_cell_spleen, Th17, "CD4+CD8+", "Neutrophil"     #
# the example below serches for everything related to th17 and Treg!!          #
...
write.xlsx(df_enhancers_blood, "output_new/mouse_chip_24h/enhancers.xlsx")
# load silencer DB
...
write.xlsx(df_sil_blood, "output_new/mouse_chip_24h/silencers.xlsx")

# Overlaps of Th17 vs Treg
data.folder <- "newest_data/chip"
Th17_peaks <- read_delim(file.path(data.folder, "2023-04-24_Th17_150_1E-15_1E-20_2_150.txt")) %>% dplyr::select(-end) %>% as.data.frame()
Th17_peaks["name"] <- sprintf("Th17_%s",seq(1:dim(Th17_peaks)[1]))
Th17_peaks_gr <- toGRanges(Th17_peaks)
Treg_peaks <- read_delim(file.path(data.folder, "2023-04-24_Treg_150_1E-5_1E-2_2_150.txt")) %>% dplyr::select(-end) %>% as.data.frame()
...
write.xlsx(unique_Treg, "output_new/mouse_chip_24h/unique_peaks_Treg_bio_vs_rosa.xlsx")
...
unique_Th17 <- ol$peaklist$Th17_peaks_gr %>% 
  ...
  unnest(cols=c(peakname))
write.xlsx(unique_Th17, "output_new/mouse_chip_24h/unique_peaks_Th17_bio_vs_rosa.xlsx")
...
common_peaks <- ol$peaklist$`Th17_peaks_gr///Treg_peaks_gr` %>% 
  ...
  unnest(cols=c(peakname))
write.xlsx(common_peaks, "output_new/mouse_chip_24h/common_peaks.xlsx")
...
# save as bed for uniqpue peaks
write_delim(chip_peaks, "output_new/mouse_chip_24h/2023-04-24_Th17_150_1E-15_1E-20_2_150.bed", delim = "\t", col_names=F)
# 
################################################################################
# Permutation test
...
pdf("output_new/mouse_chip_24h/Th17_venn_Enhancer_vs_EAseq_peaks_fdr_CD4+.pdf", width = 10, height = 10)
...
pdf("output_new/mouse_chip_24h/Th17_venn_silencer_vs_EAseq_peaks_fdr.pdf", width = 10, height = 10)
makeVennDiagram(ol,connectedPeaks = "merge",
...
# combine original chip peak file with info of overlaps
ol_df <- ol_df %>% mutate(peakname = str_extract_all(peakNames, "Th17_\\d+"))
# ol_df <- ol_df %>% mutate(peakname = str_extract(peakNames, "Th17_\\d+"))
# ol_df <- ol_df %>% mutate(overlaps_sil = str_extract(peakNames, "MS_\\d+")) # overlaps enhancer
...
pdf("output_new/mouse_chip_24h/Th17_venn_triple_OL.pdf", width = 10, height = 10)
makeVennDiagram(ol,connectedPeaks = "merge",
...
# and write to xlsx
write.xlsx(chip_peaks_combi, "output_new/mouse_chip_24h/Th17_ol_EnhancerDB_silencerDB_CD4+.xlsx")
...
write_delim(Treg_peaks, "output_new/mouse_chip_24h/2023-04-24_Treg_150_1E-5_1E-2_2_150.bed", delim = "\t", col_names=F)
################################################################################
# Permutation test
# mc.cores can be set to more than 1 in non windows environments only.
# perm_test_p <- peakPermTest(chip_peaks_ranges, Enhancers_blood_2,
...
pdf("output_new/mouse_chip_24h/Treg_venn_Enhancer_vs_EAseq_peaks_fdr_CD4+.pdf", width = 10, height = 10)
makeVennDiagram(ol, connectedPeaks = "merge",
...
pdf("output_new/mouse_chip_24h/Treg_venn_silencer_vs_EAseq_peaks_fdr.pdf", width = 10, height = 10)
makeVennDiagram(ol,connectedPeaks = "merge",
...
pdf("output_new/mouse_chip_24h/Treg_venn_triple_OL.pdf", width = 10, height = 10)
makeVennDiagram(ol,connectedPeaks = "merge",
...
ol_df <- as_tibble(overlaps.anno) %>% rowwise() %>% 
  mutate(peakNames  = paste(peakNames , collapse=',')) %>%
  ungroup()
# write.xlsx(ol_df, "output_new/mouse_chip_24h/Treg_overlap_silencerDB_fdr.xlsx")
...
# and write to xlsx
write.xlsx(chip_peaks_combi, "output_new/mouse_chip_24h/Treg_ol_EnhancerDB_silencerDB_CD4+.xlsx")
...
################################################################################
data.folder <- "newest_data/chip"
# chip_peaks <- read.xlsx(file.path(data.folder, "2023-04-24_Th17_150_1E-15_1E-20_2_150.xlsx"))
...
write.xlsx(chip_peaks_seeker@anno, "output_new/mouse_chip_24h/Th17_annotation.xlsx")
write.xlsx(chip_peaks_seeker@annoStat, "output_new/mouse_chip_24h/Th17_annotation_stat.xlsx")
# Feature distribution
# upsetplot(chip_peaks_seeker)
pdf(file = 'output_new/mouse_chip_24h/Th17_anno_plot_bar_EAseq-peaks.pdf', width=7, height=3)
plotAnnoBar(chip_peaks_seeker)
dev.off()
pdf(file = 'output_new/mouse_chip_24h/Th17_anno_plot_pie_EAseq-peaks.pdf', width=7, height=7)
plotAnnoPie(chip_peaks_seeker)
dev.off()
# Distribution of TF-binding loci relative to TSS
pdf(file = 'output_new/mouse_chip_24h/Th17_anno_plot_tss_EAseq-peaks.pdf', width=7, height=3)
plotDistToTSS(chip_peaks_seeker)
dev.off()
pdf(file = 'output_new/mouse_chip_24h/Th17_anno_plot_venn_pie_EAseq-peaks.pdf', width=7, height=7)
vennpie(chip_peaks_seeker)
dev.off()
pdf(file = 'output_new/mouse_chip_24h/Th17_anno_upsetplot_EAseq-peaks.pdf', width=7, height=7)
...
unique_Th17 <- read.xlsx("output_new/mouse_chip_24h/unique_peaks_Th17_bio_vs_rosa.xlsx")
...
write.xlsx(chip_peaks_seeker@anno, "output_new/mouse_chip_24h/Th17_unique_annotation.xlsx")
write.xlsx(chip_peaks_seeker@annoStat, "output_new/mouse_chip_24h/Th17_unique_annotation_stat.xlsx")
# Feature distribution
# upsetplot(chip_peaks_seeker)
pdf(file = 'output_new/mouse_chip_24h/Th17_unique_anno_plot_bar_EAseq-peaks.pdf', width=7, height=3)
plotAnnoBar(chip_peaks_seeker)
dev.off()
pdf(file = 'output_new/mouse_chip_24h/Th17_unique_anno_plot_pie_EAseq-peaks.pdf', width=7, height=7)
plotAnnoPie(chip_peaks_seeker)
dev.off()
# Distribution of TF-binding loci relative to TSS
pdf(file = 'output_new/mouse_chip_24h/Th17_unique_anno_plot_tss_EAseq-peaks.pdf', width=7, height=3)
plotDistToTSS(chip_peaks_seeker)
dev.off()
pdf(file = 'output_new/mouse_chip_24h/Th17_unique_anno_plot_venn_pie_EAseq-peaks.pdf', width=7, height=7)
vennpie(chip_peaks_seeker)
dev.off()
pdf(file = 'output_new/mouse_chip_24h/Th17_unique_anno_upsetplot_EAseq-peaks.pdf', width=7, height=7)
upsetplot(chip_peaks_seeker, vennpie=TRUE)
dev.off()
...
################################################################################
data.folder <- "newest_data/chip"
# chip_peaks <- read.xlsx(file.path(data.folder, "2023-04-24_Treg_150_1E-5_1E-2_2_150.xlsx"))
# chip_peaks_ranges <- toGRanges(chip_peaks)
...
write.xlsx(chip_peaks_seeker@anno, "output_new/mouse_chip_24h/Treg_annotation.xlsx")
write.xlsx(chip_peaks_seeker@annoStat, "output_new/mouse_chip_24h/Treg_annotation_stat.xlsx")
# Feature distribution
# upsetplot(chip_peaks_seeker)
pdf(file = 'output_new/mouse_chip_24h/Treg_anno_plot_bar_EAseq-peaks.pdf', width=7, height=3)
plotAnnoBar(chip_peaks_seeker)
dev.off()
pdf(file = 'output_new/mouse_chip_24h/Treg_anno_plot_pie_EAseq-peaks.pdf', width=7, height=7)
plotAnnoPie(chip_peaks_seeker)
dev.off()
# Distribution of TF-binding loci relative to TSS
pdf(file = 'output_new/mouse_chip_24h/Treg_anno_plot_tss_EAseq-peaks.pdf', width=7, height=3)
plotDistToTSS(chip_peaks_seeker)
dev.off()
pdf(file = 'output_new/mouse_chip_24h/Treg_anno_plot_venn_pie_EAseq-peaks.pdf', width=7, height=7)
vennpie(chip_peaks_seeker)
dev.off()
pdf(file = 'output_new/mouse_chip_24h/Treg_anno_upsetplot_EAseq-peaks.pdf', width=7, height=7)
upsetplot(chip_peaks_seeker, vennpie=TRUE)
dev.off()
...
# Unique peaks
################################################################################
unique_Treg <-  read.xlsx("output_new/mouse_chip_24h/unique_peaks_Treg_bio_vs_rosa.xlsx")
...
write.xlsx(chip_peaks_seeker@anno, "output_new/mouse_chip_24h/Treg_unique_annotation.xlsx")
write.xlsx(chip_peaks_seeker@annoStat, "output_new/mouse_chip_24h/Treg_unique_annotation_stat.xlsx")

# Feature distribution
# upsetplot(chip_peaks_seeker)
pdf(file = 'output_new/mouse_chip_24h/Treg_unique_anno_plot_bar_EAseq-peaks.pdf', width=7, height=3)
plotAnnoBar(chip_peaks_seeker)
dev.off()
pdf(file = 'output_new/mouse_chip_24h/Treg_unique_anno_plot_pie_EAseq-peaks.pdf', width=7, height=7)
plotAnnoPie(chip_peaks_seeker)
dev.off()
# Distribution of TF-binding loci relative to TSS
pdf(file = 'output_new/mouse_chip_24h/Treg_unique_anno_plot_tss_EAseq-peaks.pdf', width=7, height=3)
plotDistToTSS(chip_peaks_seeker)
dev.off()
pdf(file = 'output_new/mouse_chip_24h/Treg_unique_anno_plot_venn_pie_EAseq-peaks.pdf', width=7, height=7)
vennpie(chip_peaks_seeker)
dev.off()
pdf(file = 'output_new/mouse_chip_24h/Treg_unique_anno_upsetplot_EAseq-peaks.pdf', width=7, height=7)
upsetplot(chip_peaks_seeker, vennpie=TRUE)
dev.off()
